...
double* matrixVectorMultiplicationTiled(double** mat, double* vec, double* local_result, int local_rows, int cols) {
        ...
        for (int j = 0; j < cols; j += TILE_SIZE) {
            for (int ii = i; ii < i + TILE_SIZE && ii < local_rows; ii++) {
                for (int jj = j; jj < j + TILE_SIZE && jj < cols; jj++) {
                    local_result[ii] += mat[ii][jj] * vec[jj];
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    // Perform local matrix-vector multiplication with tiling
    double* local_result = allocateVector(rows_counts[world_rank]);
    matrixVectorMultiplicationTiled(local_matrix, vector, local_result, rows_counts[world_rank], vector_size);
